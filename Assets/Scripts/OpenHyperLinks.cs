...
public class OpenHyperLinks : MonoBehaviour, IPointerClickHandler
{
    ...
    public Color hoverColor = new Color(0.3613084f, 0.6392378f, 0.701f, 1f);
    ...
    protected virtual void Awake()
    {
        ...
        hasUnderline = pTextMeshPro.text.Contains("<u");
    ...
    List<Color32[]> SetLinkToColor(int linkIndex, Func<int, int, Color32> colorForLinkAndVert)
    {
        if (hasUnderline)
        {
            if (isHovering)
            {
                pTextMeshPro.text = pTextMeshPro.text.Replace("<u>", "<u color=#" + ColorUtility.ToHtmlStringRGBA(hoverColor) + ">");
            }
            else
            {
                pTextMeshPro.text = pTextMeshPro.text.Replace("<u color=#" + ColorUtility.ToHtmlStringRGBA(hoverColor) + ">", "<u>");
            }
            pTextMeshPro.ForceMeshUpdate();
        }
